...
BPLCOUNT  equ 8  
MAXDEPTH equ 8
CHKBLLINE equ 0 ;Extra bit in map for empty line for fast processing
USEMAPHEIGHT equ 1
AGA equ 1
     IFEQ DEBUG-0
...
*****************************************************************************u
		...
		;5432109876543210
DMASET	equ	%1001001111100000	; copper,bitplane,blitter DMA
...
;a3 = frame
		...
		lea    .frame, a3               ; 
		lea    EF1_PATTERNDATA0,a1		;  frmdat = EFF1_PATTERNDATA
		lea	   blarraycont,a2			;  laydat = blarraycont.data	
		lea    blarraydim, a0			;  blarraydim.width = 
		move.w -4(a2), DIMWIDTH(a0)		;  				blarraycont.width;
		move.w -2(a2), DIMHEIGHT(a0)	;  blarraydim.height =	  
										;  				blarraycont.height;
        moveq  #8-1,D0  				;  for(int i=0;i<8;i++) { 
.lp1									;    SetFrameDefault(	 
		bsr.s  SetFrameDefault			;    frmdat, laydat, frame);
		add.l  #FRMSIZE, a1				;  	 frmdat++; //Next object
		add.l  #CNTOBJSIZE, a2          ;    laydat++;
		dbf	   d0, .lp1				    ;  } 		
		;bsr.s  SetColData				;  SetColData(frame);
		;add.w  #2, (a3)					;  frame++
		bsr.w  DrawLines                ;  DrawLines(); 
...
;a3 = frame
		...
		add.w (a3), a1                 ;  //Get to right frame
		move.w FDOPOSX(a1), CNTPOSX(a2) ;  *frmdat.posx[frame] = *frmdat.posx
		move.w FDOPOSY(a1), CNTPOSY(a2) ;  '
		move.w FDOPOSXDET(a1), CNTBLPOSX(a2) ;'s
		move.w FDOPOSYDET(a1), CNTBLPOSY(a2) ;'
